...
CREATE TABLE IF NOT EXISTS orders (
  ...
  user_id         INT NULL,                                          -- 与 users.id 保持完全一致（若 users.id 是 INT UNSIGNED，这里也改为 INT UNSIGNED）
  ...
  order_status    ENUM('IP','Completed','Refunded','preorder') NOT NULL DEFAULT 'IP',
  PRIMARY KEY (id),
  UNIQUE KEY uq_orders_order_number (order_number),
  KEY idx_orders_user_created (user_id, created_at),
  KEY idx_orders_status_created (order_status, created_at),
  CONSTRAINT fk_orders_user
    FOREIGN KEY (user_id) REFERENCES users(id)   -- 注意小写 users
